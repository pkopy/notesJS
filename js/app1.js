(function () {
    ...
    let controler = {
        ...
        newWindow: function (name = '', width = 330, height = 250) {
            ...
            name = name + controler.getArrayLength();
            windowNote[name] = {
                id: name,
                timer: false,
                col: '',
        ...
        newSetPos: function () {
            ...
            let maxRow = controler.max(arrHelpRow);
            let maxCol = controler.max(arrHelpCol);
        ...
        setPos: function () {
            ...
            let x = controler.max(arrHelpCol);
            for (let i = 0; i < windows.length; i++) {
                let key = Object.keys(windows[i])[0];
        ...
        move: function (event) {
            let windows = controler.getWindows();
            let objWindow;
            ...
            let maxRow = controler.max(arrHelpRow);
            let maxCol = controler.max(arrHelpCol);
            let position = controler.findPosition(obj, maxCol);
            let allPos = [];
            for (let i = 0; i < windows.length; i++) {
                let x = windows[i][i]
                let pos = controler.findPosition(windows[i][i], maxCol)
                allPos.push({
                    id: x.id,
                    pos: pos
                });
        ...
        mouseUpremove: function (obj) {
            obj.removeEventListener('mousemove', controler.move);
            controler.setPos()
            view.render();
        },
        ...
        mouseDown: function (e) {
            ...
            if (id !== '' && !isNaN(id)) {
                ...
                body.addEventListener('mousemove', controler.move);
        ...
        changeColor1: function (start, end, elem) {
            ...
            let id = setInterval(function () {
                r = controler.changeValue(r, end.r)
                g = controler.changeValue(g, end.g)
                b = controler.changeValue(b, end.b)
                max--;
        ...
        changeColor: function (start, end, elem) {
            ...
            let id = setInterval(function () {
                ...
                if (max <= 0) {
                    clearInterval(id)
                    controler.changeHead(rgb.start, end)
                    for (let menu of menuLeft) {
        ...
        changeColorNote: function (e) {
            let obj = e;
            obj.target.removeEventListener('click', controler.changeColorNote)
            ...
            let end = controler.getColor(endId)
            controler.changeColor1(start, end, elem)
        },
        delete: function (e) {
            let windows = controler.getWindows();
        ...
        clickNote: function (e) {
            ...
            body.removeEventListener('mousedown', controler.mouseDown)
            body.removeEventListener('mouseup', controler.mouseUp)
            body.removeEventListener('mouseup', controler.mouseUpremove)
            ...
            divBack.addEventListener('click', function () {
                ...
                body.addEventListener('mousedown', controler.mouseDown)
                model.clear()
        ...
        updateTimeArea: function () {
            controler.clear();
            controler.addTimerCircle(model.timer[0]);
        },
        clear: function () {
        ...
        addDate: function (id) {
            ...
            if (id.timerY === 0) {
                ...
                id.timerType = controler.addTimerCircle
            }
            ...
            model.pushTimer(id)
            controler.clear()
            let obj = controler.objTimer(id)
            controler.addTimerCircle(id);
            viewTimer.render();

        },
        addTimerCircle: function (id) {
            let secComponent = controler.timeComponent(290, 60, 30, 7, 12, 0, id);
            let minComponent = controler.timeComponent(210, 60, 30, 7, 12, 1, id)
            let hourComponent = controler.timeComponent(130, 60, 30, 7, 30, 2, id);
            let dayComponent = controler.timeComponent(50, 60, 30, 7, 1, 3, id);
        },
    ...
    let view = {
        init: function () {
            controler.createDiv();
            window.addEventListener('resize', controler.resizeWindow);
            ...
            for (let windowNote of windows) {
                ...
                elem.addEventListener('mousedown', controler.mouseDown);
                elem.addEventListener('dblclick', controler.clickNote);
                ...
                elem.addEventListener('mouseout', function () {
                    ...
                    controler.mouseUpremove(elem)
                })
                elem.addEventListener('mouseup', function () {
                    return controler.mouseUp(elem)
                })
            }
            ...
            for (let icon of icons) {
                icon.addEventListener('mouseover', function () {
                    ...
                    icon.parentNode.parentNode.parentNode.removeEventListener('mousedown', controler.mouseDown);
                    palette.addEventListener('click', controler.changeColorNote)
                })
                icon.addEventListener('mouseout', function () {
                    ...
                    icon.parentNode.parentNode.parentNode.addEventListener('mousedown', controler.mouseDown);
                })
            }
            let icons1 = document.querySelectorAll('.xx1')
            for (let icon of icons1) {
                let palette = document.getElementById('palette')
                icon.addEventListener('click', controler.delete)
                icon.addEventListener('mouseover', function () {
                    icon.style.opacity = '1'
                    icon.appendChild(palette)
                    icon.parentNode.parentNode.parentNode.removeEventListener('mousedown', controler.mouseDown);
        ...
        render: function () {
            ...
            for (let windowNote of windows) {
                ...
                let id = setInterval(function () {
                    ...
                    left = controler.changeValue(left, windowNote[key].left, 8)
                ...
                let id1 = setInterval(function () {
                    ...
                    top = controler.changeValue(top, windowNote[key].top, 8)
    ...
    let viewHead = {
        ...
        renderHead: function (e) {
            ...
            controler.changeColor(start, end, elem)
        }
    };
    let viewTimer = {
